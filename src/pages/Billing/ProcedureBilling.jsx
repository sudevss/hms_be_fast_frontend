...
import { postCreateBills, getProcedureBillPrint, getLabBillPrint, getPharmacyBillPrint, getFacilityLogo, getFacilityDetail, getLoadDiagnosis } from "@/serviceApis";
...
const ProcedureBilling = () => {
  ...
  const handleLoadDiagnosis = async () => {
    ...
    try {
      ...
      if (data && data.procedure_items && data.procedure_items.length > 0) {
        const newRows = data.procedure_items.map((item) => {
          return {
            id: Date.now() + Math.random(),
        ...
        setRows(newRows);
        setOverallDiscount(data.procedure_discount_percent || 0);
        setSnackbarMessage("Diagnosis loaded successfully!");
        setSnackbarSeverity("success");
      } else {
        setSnackbarMessage("No procedure diagnosis found for this token.");
  ...
  const handleSave = async () => {
    ...
    // Validate that all rows have a procedure name
    const invalidRows = rows.filter((row) => !row.procedure_text || !row.procedure_text.trim());
    if (invalidRows.length > 0) {
      setSnackbarMessage("Please enter procedure name for all rows.");
      setSnackbarSeverity("warning");
      setOpenSnackbar(true);
      return;
    }
    ...
    try {
      ...
      // Fetch Pharmacy Data
      try {
        const pharmacyData = await getPharmacyBillPrint({
          token_number: diagnosisId,
          token_date: tokenDate,
        });
      ...
      const payload = {
        ...
        procedure_discount_percent: parseFloat(overallDiscount) || 0,
        procedure_items: rows.map((row) => ({
      ...
      // Open print confirmation dialog instead of clearing immediately
  ...
  const handlePrintConfirm = async () => {
    ...
    try {
      setIsSubmitting(true);
      const data = await getProcedureBillPrint({
        token_number: diagnosisId,
        token_date: tokenDate,
      });
  ...
  useEffect(() => {
    if (reprintData && reprintRef.current) {
      ...
      win.document.write(`
        <html>
          <head>
            <title>Procedure Bill Print</title>
            <style>
          ...
          <body>
            ...
            <div class="bill-info">
              ...
              <div class="info-group" style="text-align: right;">
                ...
                <div class="info-row"><span class="label">Bill Type:</span> <span class="value">Procedure Bill</span></div>
              </div>
            </div>
  ...
  return (
    <Box sx={{ p: 3, maxWidth: "1200px", margin: "0 auto" }}>
            ...
            <TableBody>
                ...
                >
                  <TableCell sx={{ py: 2, borderBottom: "1px solid #F3F4F6" }}>
                    <TextField
                      ...
                      onChange={(e) =>
                        handleChange(row.id, "procedure_text", e.target.value)
                      }
                      sx={{
              ...
              {rows.length === 0 && (
                <TableRow>
                  ...
                  >
                    No items added. Click below to add a new procedure.
                  </TableCell>
                </TableRow>
              )}
              <TableRow>
      ...
      >
        <DialogTitle id="alert-dialog-title">{"Bill Saved"}</DialogTitle>
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            Bill has been saved successfully. Do you want to print the bill now?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={handlePrintCancel} color="primary">
            No, Later
          </Button>
          <Button onClick={handlePrintConfirm} color="primary" autoFocus>
            Yes, Print
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={openReprintDialog} onClose={handleReprintClose}>
        <DialogTitle>Reprint Procedure Bill</DialogTitle>
        <DialogContent>
